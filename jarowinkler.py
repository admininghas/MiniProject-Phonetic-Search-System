...
def get_jaro_distance(first, second, winkler=True, winkler_ajustment=True, scaling=0.1):
    ...
    return jaro
# perhitungan untuk mendapat nilai jaro winkler
...
# menghitung jumlah karakter yang cocok sebelum di temukan transposisi
...
def _get_diff_index(first, second):
    ...
    return max_len
# menemukan index perbedaan karakter
...
# menghitung nilai jaro similarity

def _get_matching_characters(first, second):
    common = []
    limit = math.floor(max(len(first), len(second)) / 2) - 1

    for i, l in enumerate(first):
        left, right = int(max(0, i - limit)), int(min(i + limit + 1, len(second)))
        if l in second[left:right] and l != " ":
            common.append(l)
            second = second[0:second.index(l)] + '*' + second[second.index(l) + 1:]

    return ''.join(common)
# mencocokkan karakter antara 2 string dengan ketentuan karakter yang dicocokkan jaraknya tidak lebih dari nilai limit

def _transpositions(first, second):
    return math.floor(len([(f, s) for f, s in zip(first, second) if not f == s]) / 2.0)
# menghitung jumlah karakter transposisi dibagi 2
